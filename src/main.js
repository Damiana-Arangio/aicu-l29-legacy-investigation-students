...
function renderSummary() {
  elements.totalCount.textContent = String(tickets.length);
  elements.highCount.textContent = String(
    tickets.filter((ticket) => ticket.priority === "alta").length
  );
  elements.attentionCount.textContent = String(
    tickets.filter((ticket) => ticket.urgencyLabel === "intervento rapido").length
  );
}
...
function renderTickets() {
  ...
  const visibleTickets = tickets.filter((ticket) =>
    `${ticket.id} ${ticket.title} ${ticket.customer}`.toLocaleLowerCase("it").includes(query)
  );
